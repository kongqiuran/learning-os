...
class LearningPackage(Base):
    ...
    quota_source: Mapped[str | None] = mapped_column(String(30), nullable=True)
    quota_state: Mapped[str | None] = mapped_column(String(20), nullable=True)
    quota_units: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    quota_reserved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    quota_settled_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
