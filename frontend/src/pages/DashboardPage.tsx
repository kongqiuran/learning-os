...
export function DashboardPage() {
  ...
  if (!dashboard.data || dashboard.isError) return <StatePanel variant="error" title="课程加载失败" description="可能是网络暂时中断，你的课程和资料不会因此丢失。" action={<Button onClick={() => dashboard.refetch()}>重新加载</Button>} />
    ...
    <section className="mx-auto max-w-6xl">
      ...
      {dashboard.data.courses.length ? (
        <>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{dashboard.data.courses.map((course) => <CourseCard key={course.id} course={course} onOpen={() => openCourse(course.id)} />)}</div>
        </>
      ) : currentUser.data ? (
        <WelcomeGuide userId={currentUser.data.user.id} onCreateCourse={() => setSearchParams({ create: '1' })} />
      ) : null}
      <CreateCourseDialog open={createOpen} onClose={() => setSearchParams({}, { replace: true })} onCreated={(course) => openCourse(course.id)} />
    </section>
  )
}
