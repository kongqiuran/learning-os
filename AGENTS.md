...
2. 完成项目文件修改后，自动执行 `git add`、`git commit` 和 `git push`，并将提交与推送结果告知用户。推送成功后继续按第 5 条自动完成服务器更新和部署，不再默认让用户复制执行。如果 GitHub 连接、认证、权限或其他外部原因导致推送失败，应停止在失败步骤，直接说明需要用户处理的问题，并提供可从该步骤继续执行的命令。
